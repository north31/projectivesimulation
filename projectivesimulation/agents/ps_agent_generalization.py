...
class PSAgent(object):
	...
	def __init__(self, num_actions, num_percepts_list, generalization, gamma_damping, eta_glow_damping, policy_type, beta_softmax, hash_hash_bool, majority_vote, majority_time, number_votes):
		self.num_actions = num_actions                #action数量，常数                   2
		self.num_categories = len(num_percepts_list)  #感知种类数量                       2
		self.num_percepts_list = num_percepts_list    #每个种类的感知数量，常数列表          [2,100]
		self.n_percepts = np.prod(num_percepts_list)  #表所有数的乘积                      200
		self.generalization = generalization          #T/F                               T
		self.hash_hash_bool = hash_hash_bool          #T/F                               F
		self.majority_vote = majority_vote            #T/F                               T
		self.majority_time = majority_time            #在主要投票后，指定试错                10
		self.number_votes = number_votes              #主要投票中的投票数量                  10
		
		self.gamma_damping = gamma_damping            #伽马阻尼 γ                         0
		self.eta_glow_damping = eta_glow_damping      #η                                1
		self.policy_type = policy_type                #‘standard' or 'softmax'          standard
		self.beta_softmax = beta_softmax              #β                                1
		
		self.time = 0 # internal time for majority voting
		
		if self.generalization:
			self.max_number_hash_clips = np.prod(np.array(self.num_percepts_list) + 1)   #每个感知数量+1的乘积              303
			self.adjacency_mat_dim = self.n_percepts + self.max_number_hash_clips + self.num_actions     #邻接矩阵        200+303+2=505
			print(self.adjacency_mat_dim)
			self.percept_encoding = np.zeros([self.n_percepts, self.num_categories])           #                        200*2值为0的矩阵
			self.hash_clip_existence = np.zeros(np.array(num_percepts_list)+1, dtype=bool)   #                          3*101   F
		else:
			self.adjacency_mat_dim = self.n_percepts + self.num_actions # constructs the basic two-layered network
			
		self.h_matrix = lil_matrix((self.adjacency_mat_dim, self.adjacency_mat_dim), dtype=np.float64)    #空矩阵505*505
	...
	def percept_preprocess(self, observation): # preparing for creating a percept, creating generalizations
		percept = self.mapping_to_1d(observation)      #=1
		print(percept)
	...
	def mapping_to_1d(self, observation):           #将环境感知由多个变成一个感知 observation=action,reward
	...
	def generalization_function(self, observation): # generalization function that compares all percepts, creates #-clips and edges
		for i_percept in range(self.n_percepts):
			print(self.h_matrix)
			print(i_percept)
			print(np.sum(self.h_matrix[:, i_percept]))
